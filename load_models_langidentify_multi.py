...
def predict_common(texts, model, label):
    samples = []
    a, b, c = [], [], []
    for text in texts:
    ...
    preds = model([tf.constant(a), tf.constant(b), tf.constant(c)])
    ...
    output = [label[i] for i in class_index]
...
def _all(texts):
    label = labels
    prediction = predict_common(texts, model, label)
    return prediction
...
def language_identify(texts):
    ...
    #sentences = [text]
    #print ('here 1', sentences)
    #output_list = []
    #for sentence in sentences:
    p_all = _all(sentences)
    #print (p_all)
...
text = ['सुजुकी जिम्नी इलेक्ट्रिक को पहले यूरोपीय बाजार में लॉन्च किया जाएगा. इसके बाद 5 दरवाजे वाली इलेक्ट्रिक ऑफ-रोडर के रूप में भारतीय बाजार में अपना रास्ता बनाएगी. ','ગુજરાત પંચાયત વિભાગ દ્વારા જુનિયર કલાર્ક પરીક્ષા આજે સવારે 11 કલાકે યોજવામાં આવી હતી.']
